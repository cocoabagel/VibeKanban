...
var swiftVersion = ProcessInfo.processInfo.environment["PROJECT_SWIFT_VERSION"] ?? "6.2"
...
if isSingleFile {
    ...
    shell(
        "fd . --full-path \(filePath) -e swift -x \(swiftlint) --fix --format --config \(swiftLintConfig) {}"
    )
}
